...
Connection.prototype.upsert = function (type, records, extIdField, options, callback) {
  ...
  if (records.length > MAX_DML_COUNT && options.allowRecursive) {
    var self = this;
    return self.upsert(type, records.slice(0, MAX_DML_COUNT), options).then(function (rets1) {
      return self.upsert(type, records.slice(MAX_DML_COUNT), options).then(function (rets2) {
        return rets1.concat(rets2);
      });
    });
  }
  ...
  var url = [self._baseUrl(), "sobjects", sobjectType, null].join('/');
  return self.request({
    method: 'POST',
    url: url,
    body: JSON.stringify({
